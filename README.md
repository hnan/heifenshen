# 嘿分身（HeiFenshen）— Mac 微信多开 / 微信双开工具

> Mac 下微信、企业微信多开工具。无需破解、不改文件，利用 macOS 原生沙盒能力实现独立实例。

[![最新版本](https://img.shields.io/badge/版本-v1.7.1-blue)](https://github.com/hnan/heifenshen/releases)
[![macOS](https://img.shields.io/badge/macOS-12.0%2B-brightgreen)](https://github.com/hnan/heifenshen)
[![免费使用](https://img.shields.io/badge/价格-免费-orange)](https://heiweixin.com)
[![下载](https://img.shields.io/badge/下载-6MB-lightgrey)](https://www.heiweixin.com)

[官网](https://heiweixin.com) · [下载最新版](https://www.heiweixin.com) · [问题反馈](https://github.com/hnan/heifenshen/issues)

![嘿分身界面](screenshot.png)

---

## 简介

**嘿分身**利用 macOS 原生沙盒能力，让你在 Mac 上同时运行多个微信、企业微信实例，无需破解 App、无需修改任何文件，微信或系统升级后依然可用。

无论你要的是微信多开、微信双开，还是给工作号单独开一个微信分身，装上就能用。适合需要同时管理多个账号的用户：工作账号 + 个人账号，或多个业务微信号。

## 功能特性

- **无限多开** — 同时运行任意数量的微信、企业微信实例
- **升级后仍可用** — 独立于 App 版本运行，无需重新配置
- **隐私安全** — 利用 macOS 沙盒创建独立实例，不修改任何 App 文件
- **轻量免费** — 安装包仅约 6MB，基础功能完全免费

## 使用方法

1. [下载安装包](https://www.heiweixin.com)，拖入应用程序文件夹
2. 打开嘿分身，从列表中选择要多开的应用（微信 / 企业微信）
3. 点击「启动分身」，在新实例中登录另一个账号

## 系统要求

| 项目 | 要求 |
|------|------|
| 系统 | macOS 12.0 及以上 |
| 支持应用 | 微信（4.0+）、企业微信 |
| 芯片 | Intel 与 Apple Silicon 均支持 |
| 安装包大小 | 约 6MB |

## 更新日志

**v1.7.1** · 2026-09-17
- 修复微信升级到 4.1.15.19 后，分身无法打开的问题
- 新建分身时可选择图标颜色，在 Dock 和切换窗口时一眼区分
- 修复分身图标在 Dock、访达中仍显示原版微信图标的问题
- 列表中标记为「应用损坏」的分身，点击「修复」即可恢复使用

**v1.7.0** · 2026-09-17
- 修复微信 4.1.15 下新建分身无法打开的问题
- 升级后，列表中标记为「应用损坏」的旧分身点击「修复」即可恢复使用

**v1.6.9** · 2026-08-14
- 优化网络连接稳定性

**v1.6.8** · 2026-05-24
- 优化网络不好时的稳定性

[查看完整更新日志 →](https://heiweixin.com/changelog)

## 常见问题

**会被微信封号吗？**  
嘿分身使用 macOS 原生沙盒机制，与破解方案不同，不修改微信文件，风险极低。

**微信或 macOS 升级后还能用吗？**  
可以。嘿分身独立于 App 版本运行，升级后仍可继续使用。

**需要关闭 SIP（系统完整性保护）吗？**  
不需要。嘿分身完全依赖 macOS 原生能力，无需修改系统设置。

**支持 Apple Silicon（M 系列芯片）吗？**  
支持，Intel 和 Apple Silicon 均可使用。

更多问题请访问[官网 FAQ](https://heiweixin.com/#faq)

## 下载

前往官网下载最新版本：**[heiweixin.com](https://heiweixin.com)**

---

## English

**嘿分身 (HeiFenshen)** lets you run multiple WeChat / WeCom instances simultaneously on macOS, using native sandbox capabilities — no patching, no file modification required. Works after app or system upgrades.

**Features**: Unlimited instances · Upgrade-proof · Privacy-safe · Free & lightweight (6MB)  
**Requirements**: macOS 12.0+, WeChat 4.0+, Intel & Apple Silicon  
**Download**: [heiweixin.com](https://heiweixin.com)

---

© 2026 嘿分身. 有问题欢迎提 [Issue](https://github.com/hnan/heifenshen/issues)。
